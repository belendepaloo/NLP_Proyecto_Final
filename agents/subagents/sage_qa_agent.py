...
Lee el texto del chunk del disco (run_id, chunk_id) en vez de recibirlo en el mensaje de
la tarea, y persiste el resultado del paraphraseo ahi tambien -- mismo patron que
...
SYSTEM_PROMPT = """Sos el agente de QA de SAGE. Recibis un run_id y una lista de
chunk_id. Antes de arrancar, llama a list_run_artifacts(run_id) UNA vez y guarda esa
lista -- la vas a necesitar para cada chunk_id sin volver a pedirla. Para CADA
chunk_id:
...
1. Fijate en la lista de "curation" que ya tenes si "chunk_{chunk_id}.json" esta ahi
   ...
   texto. Si SI esta, llama a read_run_artifact(run_id, "curation", f"chunk_{chunk_id}")
...
4. Si "passed" es true para todos los segmentos narrativos: el chunk esta listo. Llama
   a write_run_artifact(run_id, "sage", f"paraphrase_{chunk_id}", {"chunk_id":...,
...
5. Si algun segmento fallo: decidi entre
   ...
   - DESCARTAR el chunk si sigue fallando despues de reintentar (anotalo en tu resumen
     ...
     para ese chunk -- su ausencia en runs/<run_id>/sage/ es la señal de que se
     descarto).
...
sage_qa_subagent = {
    ...
    "tools": [list_run_artifacts, read_run_artifact, write_run_artifact, run_sage_tool, sage_quality_check],
}
